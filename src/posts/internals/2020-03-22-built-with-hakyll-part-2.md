...
subtitle: Usefull  Hakyll Contexts
tags: internal, technical, haskell
description: This site uses a bunch of custom fields defined that I found usefulor fun to build.
...
# Built with Hakyll -- Part 02: Fields
...
```
For each  `*.md` file inside `posts/` an `.html` file is generated by compiling the docuemnt using pandoc (which turnsthe markup it into html) and including this content in a template, namely the `templates/post.html` template, by loading the template and applying it with  a given `Conetxt`.
